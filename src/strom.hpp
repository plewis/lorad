...
namespace strom {
    ...
    class Strom {
        ...
        private:
            ...
            //std::pair<double,double>                estimateRingVolume(unsigned k);
    ...
    inline void Strom::processCommandLineOptions(int argc, const char * argv[]) {
        ...
        desc.add_options()
            ...
            ("nstones", boost::program_options::value(&_nstones)->default_value(0),                   "use this many steppingstones to compute marginal likelihood with the steppingstone method (set nstones to 0 to avoid doing steppingstone)")
        ...
        // If number of stones is greater than 0, then set _num_chains to that value
        if (_nstones > 0) {
            std::cout << (boost::format("\nNumber of chains was set to the specified number of stones (%d)\n") % _nstones) << std::endl;
            _num_chains = (unsigned)_nstones;
        }
...
//        }
    ...
    inline void Strom::sample(unsigned iteration, Chain & chain) {
        if (_nstones > 0) {
            ...
            if (time_to_sample || time_to_report) {
                ...
                if (time_to_sample) {
                    // Save parameters and tree tree to files
    ...
    inline void Strom::standardizeParameters() {
        ...
        std::cout << "\n_mode_transformed: " << _mode_transformed.format(fmt) << "\n";
        std::cout << boost::format("best_log_kernel = %.5f\n") % best_log_kernel;
        std::cout << boost::format("best_iteration  = %.5f\n") % _best_iteration;
...
#if 0
        // check to make sure we can reconstitute _S
        eigenMatrixXd_t S;
        S.resize(_nparams, _nparams);
        S = V*L.asDiagonal()*V.transpose();
        double ss = 0.0;
        for (unsigned ii = 0; ii < _nparams; ii++)
            for (unsigned jj = 0; jj < _nparams; jj++)
                ss += pow(S(ii,jj) - _S(ii,jj),2.0);
        std::cerr << boost::format("\ndebug ~~> sum of squared differences between _S and S = %.5f\n") % ss;
#endif
...
#if 0
        // Calculate variance-covariance matrix for standardized sample
        eigenVectorXd_t std_mean = eigenVectorXd_t::Zero(_nparams);
        for (auto & v : _standardized_parameters) {
            std_mean += v._param_vect;
        }
        std_mean /= _nsamples;
        ...
        eigenMatrixXd_t SS;
        SS.resize(_nparams, _nparams);
        for (auto & v : _standardized_parameters) {
            eigenVectorXd_t  x = v._param_vect - std_mean;
            SS += x*x.transpose();
        }
        SS /= _nsamples - 1;
        
        std::cout << "SS:\n" << SS << "\n";
        std::cout << "SS trace = " << SS.trace() << "\n";
        std::cout << "SS sum - trace = " << (SS.sum() - SS.trace()) << "\n";
        std::cout << std::endl;
#endif
    ...
    inline void Strom::calcMarginalLikelihood() {
        if (_nstones > 0) {
            ...
            std::vector<std::tuple<double, double, double> > log_ratio;
            for (auto & c : _chains) {
                log_ratio.push_back(std::make_tuple(c.getHeatingPower(), c.getNextHeatingPower(), c.calcLogSteppingstoneRatio()));
            }
            ...
            _output_manager->outputConsole(boost::str(boost::format("%20s %20s %20s %20s") % "beta" % "nextbeta" % "log(ratio)" % "cumulative"));
            double log_marginal_likelihood = 0.0;
            for (auto p : log_ratio) {
                double beta = std::get<0>(p);
                double nextbeta = std::get<1>(p);
                double logratio = std::get<2>(p);
                log_marginal_likelihood += logratio;
                _output_manager->outputConsole(boost::str(boost::format("%20.5f %20.5f %20.5f %20.5f") % beta % nextbeta % logratio % log_marginal_likelihood));
            }
        ...
        try {
            ...
            if (_nstones == 0) {
                ...
                if (_fixed_tree_topology && _use_gss) {
                    ...
                    // Read in contents of hpdml.conf file
                    std::ifstream inf("hpdml.conf");
                    std::string file_contents( (std::istreambuf_iterator<char>(inf)), std::istreambuf_iterator<char>() );
                    inf.close();
                    
                    // Delete any existing reference distribution commands in hpdml.conf
                    assert(file_contents.size() > 0);
#if defined(USE_BOOST_REGEX)
                    boost::regex refdistsection("# Reference distributions start.+# Reference distributions end");
                    file_contents = boost::regex_replace(file_contents, refdistsection, std::string(""));
#else
                    std::regex refdistsection("# Reference distributions start[\\S\\s]+# Reference distributions end");
                    file_contents = std::regex_replace(file_contents, refdistsection, "");
#endif
                    boost::trim(file_contents);
                    
                    // Append new reference distribution commands in hpdml.conf
                    std::ofstream outf("hpdml.conf");
                    outf << file_contents;
                    outf << "\n\n# Reference distributions start\n";
                    outf << s;
                    outf << "# Reference distributions end\n";
                    outf.close();
                }
#endif
            }
        ...
        for (unsigned k = 0; k < _nshells; ++k) {
            ...
            while (A._ndarts_thrown < _ndarts) {
                ...
                if (k == 0) {
                    ...
                    //temporary!
                    if (!test_dart_thrown) {
                        dart = Eigen::VectorXd::Zero(_nparams);
                        kernel = calcLogTransformedKernel(dart);
                        std::cerr << boost::format("test dart at zero: norm=%.5f, log-kernel=%.5f\n") % dart.norm() % kernel.logKernel();
                        test_dart_thrown = true;
                    }
                }
            }
            tmpf.close();
...
#if defined(POLNEW)
//    inline std::pair<double,double> Strom::estimateRingVolume(unsigned) {
//        WorkingSpaceSubset & A = _A[k];
...
//
//            double znorm = z.norm();
//            Eigen::VectorXd dart = z*(A._norm_max*u/znorm);
//            Kernel k = calcLogTransformedKernel(dart);
//
//            //temporary!
//            //Kernel tmpk = calcLogTransformedKernel(_mode_transformed);
//            //tmpk._title = "Kernel for _mode_transformed point";
//            //std::cerr << tmpk << std::endl;
//
//            double qdart = k.logKernel();
//            if (qdart > A._lower_logq && qdart <= A._upper_logq) {
//                darts_inside++;
//            }
//        }
...
//    }
    ...
    inline double Strom::histogramMethod() {
        ...
        for (unsigned k = 0; k < _nshells; ++k) {
            //std::pair<double,double> p = estimateRingVolume(k);
    ...
    // and output them in a file that can be compared to params.txt (after using "sort doof.txt")
    inline void Strom::debugCheckStandardization() {
        std::ofstream outf("doof.txt");
        Chain & chain = _chains[0];
        for (auto & s : _standardized_parameters) {
            Kernel k = calcLogTransformedKernel(s._param_vect);
            double log_kernel_stored = s._kernel.logKernel() - k.logJacobian();
            double kernel_check = fabs(k._log_likelihood + k._log_prior - log_kernel_stored);
            TreeManip::SharedPtr tm = chain.getTreeManip();
            double TL = tm->calcTreeLength();
            Model::SharedPtr model = chain.getModel();
            outf << boost::format("%12d %12.5f %12.5f %12.5f %12.5f %s\n") % s._iteration % kernel_check % k._log_likelihood % k._log_prior % TL % model->paramValuesAsString("\t");
        }
        outf.close();
    }
#endif
